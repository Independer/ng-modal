...
export class ModalClosedEventArgs {
  constructor(
    public readonly reason: ModalCloseReason,
...
export class ModalComponent implements OnInit, OnDestroy {
  ...
  get classModalClosing() {
    return this.isClosing;
  }

  @HostBinding("style.display")
  get styleDisplay() {
    return this.isOpening || this.isOpened ? "block" : "none";
  }

  @HostBinding("attr.hidden")
  @HostBinding("attr.area-hidden")
  get attrHidden() {
    return !this.isOpened && !this.isOpening ? "" : undefined;
  }
  ...
  protected doClose(reason: ModalCloseReason, result?: any) {
    ...
    if (
      reason === ModalCloseReason.BrowserBackNavigation &&
      this.routeBehavior
    ) {
  ...
  protected onClosed(reason: ModalCloseReason, result?: any) {
    ...
    if (
      this.routeBehavior &&
      reason !== ModalCloseReason.BrowserBackNavigation &&
      reason !== ModalCloseReason.Destroy
    ) {
      window.history.back();
    }
  }
